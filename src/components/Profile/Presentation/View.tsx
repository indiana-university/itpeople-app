...
import { IPerson } from "src/components/types";
    ...
    <Content className="rvt-bg-white">
      <Row data-testid="profile-page" className="rvt-p-top-lg">
        <Col md={6} className="rvt-p-all-lg">
          <Loader {...person}>{person && person.data && <PersonDetails person={person.data} canEdit={true} editJobClasses={editJobClasses} closeModal={closeModal} save={save} />}</Loader>
        </Col>
        <Col md={6} className="rvt-p-all-lg">
          <Loader {...memberships}>
